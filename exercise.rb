...
def add_views
  @article_hash[:response][:results].each_index do |n|
    @article_hash[:response][:results][n][:views] = 0
  end
end

def read_article
  read_title = @article_hash[:response][:results].sample
  n = @article_hash[:response][:results].index(read_title)
  @article_hash[:response][:results][n][:views] += 1
  return @article_hash[:response][:results][n]
end

def display_views
  @article_hash[:response][:results].each_index do |n|
    puts "=========================================================="
    puts "Title: #{@article_hash[:response][:results][n][:webTitle]}"
    puts "Count: #{@article_hash[:response][:results][n][:views]}"
  end
end

add_views

20.times {puts "Randomly select article: #{read_article[:webTitle]}"}
